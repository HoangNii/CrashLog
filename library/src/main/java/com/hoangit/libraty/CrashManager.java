...
public final class CrashManager {
        ...
        try {
            if (context == null) {
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    Log.e(TAG, "CrashManager was already installed, doing nothing!");
                } else {
                    if (oldHandler != null && !oldHandler.getClass().getName().startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
                        Log.e(TAG, "IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this is correct? If you use a custom UncaughtExceptionHandler, you must initialize it AFTER CrashManager! Installing anyway, but your original handler will not be called.");
                    }
    ...
    public static CrashConfig getConfigFromIntent(@NonNull Intent intent) {
        CrashConfig config = (CrashConfig) intent.getSerializableExtra(CrashManager.EXTRA_CONFIG);
    ...
    public static String getActivityLogFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(CrashManager.EXTRA_ACTIVITY_LOG);
    }
